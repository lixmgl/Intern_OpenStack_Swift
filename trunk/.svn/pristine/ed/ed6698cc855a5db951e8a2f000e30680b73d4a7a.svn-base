...
class StatsdServer(object):
    ...
    def report_stats(self, payload):
        ...
        """
        self.logger.debug ("reporting stats -> {\n%s}" % payload)
        try:
    ...
    def stats_flush(self):
        ...
        while True:
            ...
            self.logger.debug( "seen %d stats so far." % self.stats_seen)
            self.logger.debug( "current counters: %s" % self.counters )
            ...
            for item in self.gauges:   
                gauge_stats = 'gauge.%s %s %s\n' % (item,
                                    self.gauges[item], tstamp)
                payload.append(gauge_stats)
                self.gauges[item] = 0
    ...
    def process_timer(self, key, fields):
        ...
        try:
            ...
            self.logger.exception("error decoding timer event: %s" % err)
    ...
    def process_counter(self, key, fields):
        ...
        try:
            ...
            self.logger.exception("error decoding counter event: %s" % err)

    def process_gauge(self, key, fields):
        """
        Process a received gauge event

        :param key: Key of gauge
        :param fields: Received fields
        """
        try:
            counter_value = float(fields[0] or 1)
            
            if key not in self.gauges:
                self.gauges[key] = 0
            self.gauges[key] = counter_value
    ...
    def decode_recvd(self, data):
        ...
        if len(bits) == 2:
            ...
            self.logger.debug( "got key: %s" % key)
            fields = bits[1].split("|")
            ...
            if field_count >= 2:
                if fields[1] == "ms":
                    ...
                    self.logger.error( "error: unsupported stats type")
                    self.logger.error( "key -> %s\nfields ->%s" % (key, fields))
            else:
                self.logger.error( "error: not enough fields received")
        else:
            self.logger.error( "error: invalid request")
    ...
    def run(self):
        ...
        if self.combined_events:
            self.logger.debug( "combined_events mode enabled" )
            while 1:
